...
n = 100 ; num_method = 5 ;   Ns = 100:100:1000;
nums = 100;  snr = 40 ;  sp = 0.1 ; mnRatio = 0.6;
filename = ['ResultsData\Sparse_Signal_Recovery\Gaussian_',num2str(snr),'dB_n',num2str(Ns(end)),'_4.mat'] ;
...
for jj=1:Nmax
    ...
    for kk=1:nums
        % generate the data
        Phi = randn(m,n);
        w = genGaussianSignal(fix(sp*n),n) ;
        y = Phi * w  ;
...
%         ii = ii+1 ;
%         tic
%         xhat(:,ii) = EM_SBL(y_noise, Phi, paras) ;
%         time_SBL(kk,jj,ii) = toc;
%         errs(kk,jj,ii) = norm(xhat(:,ii)-w)/norm(w) ;
%         if norm(xhat(:,ii)-w,'inf')/norm(w)> tau0
%             num_fails(jj,ii) = num_fails(jj,ii)+1;
...
set(gca,'FontSize',12)
legend('GFSBL','LFSBL','LPBCS','GGAMP-SBL','EM-SBL','RVM-SBL')
